...
-spec start_link(etcdgun:client(), LeaseId :: integer()) -> gen_server:start_ret().
start_link(Client, LeaseId) ->
    gen_server:start_link(?MODULE, {Client, LeaseId}, []).
...
init({Client, LeaseId}) ->
    case new_request(Client, LeaseId) of
        {ok, State} -> {ok, State};
        {error, lease_expired} -> {stop, {shutdown, lease_expired}};
        {error, Reason} ->
            ?LOG_ERROR("Failed to start keepalive for lease ~p: ~p", [LeaseId, Reason]),
            timer:sleep(1000), %% Sleep to avoid immediate retry
            {stop, Reason}
    end.
...
handle_info({timeout, TRef, retry}, #state{client = Client, retry_ref = TRef} = State) ->
    case new_request(Client, State#state.lease_id) of
...
handle_info({'DOWN', MRef, process, _ConnPid, Reason}, #state{monitor_ref = MRef} = State) ->
...
handle_continue(process_data, #state{buf = Buf, stream = Stream} = State) ->
    case egrpc_stream:parse_msg(Stream, Buf) of
        more -> {noreply, State};
        %% TODO: Handle error, such as timeout or parse error, or network error
        % {error, Reason} ->
        %     ?LOG_ERROR("Failed to parse message: ~p", [Reason]),
        %     {noreply, cancel_scheduler(State#state{retry_ref = undefined}), {continue, {retry, Reason}}};
        {ok, Stream1, Msg, Rest} ->
            process_response(Msg, State#state{buf = Rest, stream = Stream1})
    end.

terminate(Reason, #state{stream = Stream} = _State) ->
...
process_response(#{'ID':= _LeaseId, 'TTL' := TTL}, State) when TTL =< 0 ->
    {stop, {shutdown, lease_expired}, State};
process_response(#{'ID':= _LeaseId, 'TTL' := TTL}, State) ->
    {ok, TRef} = schedule_keepalive(TTL),
